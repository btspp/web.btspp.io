...
(function(jQuery) {
  ...
  JSONFormatter = (function() {
    ...
    JSONFormatter.prototype.valueToHTML = function(value, level, output_tips) {
      var valueType;
      if (level == null) {
        level = 0;
      }
      ...
      return this["" + valueType + "ToHTML"].call(this, value, level, output_tips);
    };
    ...
    JSONFormatter.prototype.stringToHTML = function(value, level, output_tips) {
      var multilineClass, newLinePattern;
      if (/^(http|https|file):\/\/[^\s]+$/i.test(value)) {
        return "<a href=\"" + (this.htmlEncode(value)) + "\"><span class=\"q\">\"</span>" + (this.jsString(value)) + "<span class=\"q\">\"</span></a>";
      } else if (/^\d+\.\d+\.\d+$/i.test(value)) {
        if (this.options.oidmemos && output_tips != null) {
          var memo = this.options.oidmemos[value];
          if (memo) {
            output_tips.push("<span class=\"oidmemo\">" + memo + "</span>");
          }
        }
        return "<a href=\"/#/oid/" + (this.htmlEncode(value)) + "\"><span class=\"q\">\"</span>" + (this.jsString(value)) + "<span class=\"q\">\"</span></a>";
      } else {
        multilineClass = '';
    ...
    JSONFormatter.prototype.arrayToHTML = function(array, level) {
      var collapsible, hasContents, index, numProps, output, value, _i, _len, output_tips;
      if (level == null) {
        level = 0;
      }
      hasContents = false;
      ...
      for (index = _i = 0, _len = array.length; _i < _len; index = ++_i) {
        ...
        hasContents = true;
        output_tips = [];
        output += '<li>' + this.valueToHTML(value, level + 1, output_tips);
        if (numProps > 1) {
          output += ',';
        }
        if (output_tips.length > 0) {
          output += output_tips[0];
        }
        output += '</li>';
        numProps--;
      }
    ...
    JSONFormatter.prototype.objectToHTML = function(object, level) {
      var collapsible, hasContents, key, numProps, output, prop, value, output_tips;
      if (level == null) {
        level = 0;
      }
      hasContents = false;
      ...
      for (prop in object) {
        ...
        output_tips = [];
        output += "<li><a class=\"prop\" href=\"javascript:;\"><span class=\"q\">\"</span>" + key + "<span class=\"q\">\"</span></a>: " + (this.valueToHTML(value, level + 1, output_tips));
        if (numProps > 1) {
          output += ',';
        }
        if (output_tips.length > 0) {
